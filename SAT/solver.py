        ...
        while clause < size:
            ...
            while literal < size1:
                if(sorted_list[clause][literal] == lit):
                    ...
                    if(len(sorted_list[clause]) == 0): return [0],[]
                    break
                literal += 1
            if(flag1 == True):
...
#         if(len(temp) == literal_count): return temp
#     return []
def dpll_algorithm(formula, unassiged, result, literal_count):
    #formula,result1 = pure_literal_elimination(formula)
    if(formula == [0]): return []
    ...
    if(len(formula) == 0): return []
    x = unassiged[:]
    unassiged  = list(set(x).difference(set(result1)))
    size = len(unassiged)
    for i in range(size):
        print("forward{}".format(unassiged[i]))
        newResult = result[:]
        newResult.append(unassiged[i])
        newUnassigned = unassiged[:]
        newFormula = formula[:]
        newFormula = findNewFormula(newFormula,newUnassigned[i])
        #print("newFormula{}".format(newFormula))
        #print(newFormula)
        newUnassigned.pop(i)
        temp = dpll_algorithm(newFormula, newUnassigned, newResult, literal_count)
        if(len(temp) == literal_count): return temp 
        print("backward{}".format(-unassiged[i]))
        newResult = result[:]
        newResult.append(-unassiged[i])
        newUnassigned = unassiged[:]
        newFormula = formula[:]
