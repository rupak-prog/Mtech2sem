...
void caseA(){
    ...
    poisson_distribution<long int> distribution1 (arrivalRate); 
    ...
    for (long int i = 0; i < population; ++i)
    {
        long double number = Exponential(departRate);
        ST[i] = number;
...
void caseC(){
    ...
    for (long int i=0; i<population; ++i)
    {
        long double number = Exponential(departRate); //exponential function is declared above the main
        ST[i] = number;
        chrono::duration<long double> period (number);     
    } 
