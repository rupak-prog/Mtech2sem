...
do 
    ...
    read word
    if [ -z "$word" ];then #-z string - True if the string length is zero.
      echo "word is empty"
    elif [[ $word =~ [0-9] ]];then #string1 =~ regex- The regex operator returns true if the left operand matches the extended regular expression on the right.
      echo "word should not contains numbers please enter again!!!"
    elif [[ ! $word =~ ^[[:alnum:]]+$ ]];then #not alphanumeric
